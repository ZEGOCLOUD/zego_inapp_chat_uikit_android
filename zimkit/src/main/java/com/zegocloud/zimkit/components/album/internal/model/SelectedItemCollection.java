package com.zegocloud.zimkit.components.album.internal.model;
...
public class SelectedItemCollection {
        ...
        if (maxSelectableReached()) {
            ...
            try {
                cause = mContext.getResources().getQuantityString(
                        R.plurals.zimkit_error_over_count,
                        maxSelectable,
                        maxSelectable
                );
            } catch (Resources.NotFoundException e) {
                cause = mContext.getString(
                        R.string.zimkit_error_over_count,
                        maxSelectable
                );
            ...
            return new IncapableCause(IncapableCause.TOAST_DIALOG, cause);
        } else if (overFileSize(item)) {
            return new IncapableCause(IncapableCause.TOAST_DIALOG, mContext.getString(R.string.zimkit_album_over_size));
        } else if (typeConflict(item)) {
            return new IncapableCause(mContext.getString(R.string.zimkit_error_type_conflict));
        }
    ...
    public boolean overFileSize(Item item) {
        SelectionSpec spec = SelectionSpec.getInstance();
        float size = PhotoMetadataUtils.getSizeInMB(item.size);
        if (item.isImage() && size >= spec.imageMaxSize) {
            return true;
        } else if (item.isVideo() && size >= spec.videoMaxSize) {
            return true;
        } else {
            return false;
        }
