...
public class GroupService {
    ...
    public void createGroup(String groupName, String groupId, List<String> inviteUserIDs, CreateGroupCallback callback) {
        ...
        ZIMKitCore.getInstance().zim().createGroup(info, inviteUserIDs, (groupInfo, userList, errorUserList, errorInfo) -> {
            if (callback != null) {
    ...
    public void inviteUsersToJoinGroup(List<String> userIDs, String groupID, InviteUsersToJoinGroupCallback callback) {
        ZIMKitCore.getInstance().zim().inviteUsersIntoGroup(userIDs, groupID, (groupID1, userList, errorUserList, errorInfo) -> {
            if (callback != null) {
                ArrayList<ZIMKitGroupMember> groupMembers = new ArrayList<>();
    ...
    private ZIMKitGroupInfo transGroupInfo(ZIMGroupFullInfo groupInfo) {
    ...
    private ZIMKitGroupMember transGroupMember(ZIMGroupMemberInfo info) {
        ZIMKitGroupMember member = new ZIMKitGroupMember();
        if(info == null){
            return member;
        }
        ...
        member.setAvatarUrl(info.memberAvatarUrl);
